...
select hiredate  ,substr(hiredate, 0, 5) from emp;
...
select trunc(sysdate - to_date('2020,01,01','YY.MM.DD'))as"2020년 1월 1일 이후" 
from dual; 
...
-- 관리자를 알 수 없는 사원 where mgr is not null
-- 급여가 2000미만인 그룹 제외  having
...
select job , deptno,
nvl(decode(deptno,10,sum(sal)),0) "부서 10",
nvl(decode(deptno,20,sum(sal)),0) "부서 20",
nvl(decode(deptno,30,sum(sal)),0) "부서 30",
sum(sal) "총액"
from emp
